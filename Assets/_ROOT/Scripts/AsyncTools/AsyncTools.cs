...
public static class AsyncTools
{
    ...
    private class AsyncOperationAwaiter : Awaiter
    {
        ...
        public override void OnCompleted(Action action) => NextFrameHelper.Enqueue(() => CheckForCompletion(action));

        private void CheckForCompletion(Action action)
        {
            if (asyncOp.isDone)
            {
                action();
            ...
            {
                NextFrameHelper.Enqueue(() => CheckForCompletion(action));
            }
        }
    }
