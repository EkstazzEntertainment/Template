...
public class Reporter : MonoBehaviour
{
    ...
    void addSample()
    {
        ...
        sample.loadedScene =
            0; //(byte)UnityEngine.SceneManagement.SceneManager.GetActiveScene().buildIndex;//(byte)Application.loadedLevel ;
    ...
    public void Initialize()
    {
        if (!created)
        {
            ...
            scenes = new string[UnityEngine.SceneManagement.SceneManager
                .sceneCount]; //new string[ Application.levelCount ];
            currentScene =
                UnityEngine.SceneManagement.SceneManager.GetActiveScene().name; //Application.loadedLevelName;
    ...
    void DrawInfo()
    {
        ...
        GUILayout.Space(size.x);
        GUILayout.Label(fps.ToString("0.000"), nonStyle, GUILayout.Height(size.y));
        GUILayout.FlexibleSpace();
        GUILayout.EndHorizontal();
    ...
    void drawInfo_enableDisableToolBarButtons()
    {
        ...
        tempRect = GUILayoutUtility.GetLastRect();
        GUI.Label(tempRect, fps.ToString("0.000"), lowerLeftFontStyle);
    ...
    void DrawLogs()
    {
        ...
        for (int i = startIndex; (startIndex + index) < endIndex; i++)
        {
            ...
            if (showFps)
            {
                tempContent.text = sample.fps.ToString("0.000");
            ...
            if (log == selectedLog)
            {
                ...
                if (showFps)
                {
                    ...
                    GUI.Label(fpsLabelRect, sample.fps.ToString("0.000"), currentLogStyle);
                }
            }
            else
            {
...
//  Vector3 tempVector2 ;
    ...
    void drawGraph()
    {
        ...
        if (currentFrame != -1 && currentFrame < samples.Count)
        {
            ...
            GUILayout.Box(showFpsContent, nonStyle, GUILayout.Width(size.x), GUILayout.Height(size.y));
            GUILayout.Label(selectedSample.fps.ToString("0.000"), nonStyle);
            GUILayout.Space(size.x);
    ...
    void Update()
    {
        ...
        if (string.IsNullOrEmpty(scenes[ /*SceneManager.GetActiveScene().buildIndex*/0]))
        {
            scenes[ /*SceneManager.GetActiveScene().buildIndex*/0] = SceneManager.GetActiveScene().name;
        }
    ...
    void OnSceneLoaded(Scene scene, LoadSceneMode mode)
    {
        ...
        //scenes = new string[sceneCount]; //new string[ Application.levelCount ];
        //currentScene = UnityEngine.SceneManagement.SceneManager.GetActiveScene().name;//Application.loadedLevelName;
