namespace Ekstazz.Configs
{
    ...
    internal abstract class ConfigPropertyFiller
    {
        ...
        public virtual void Fill(object obj, PropertyInfo propertyInfo, string propertykey, bool useMultiConfig = false)
        {
            ...
            {
                throw new Exception($"No such key in ConfigFetcher: {propertykey}. The property will be unassigned.",
                    e);
            }
            catch (Exception e)
            {
                throw new Exception($"Failed to fill property with key \"{propertykey}\". The property will be unassigned.",
                    e);
            }
        }
