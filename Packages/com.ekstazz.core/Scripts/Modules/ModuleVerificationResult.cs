namespace Ekstazz.Core.Modules
{
    ...
    public class ModuleVerificationResult
    {
        ...
        public static ModuleVerificationResult Logs(List<string> messages) => FromMessages(messages.Select(m => new VerificationMessage(MessageSeverity.Log, m)).ToList());
        public static ModuleVerificationResult Warning(string message) => Warnings(new List<string>() { message });
        public static ModuleVerificationResult Warnings(List<string> messages) => FromMessages(messages.Select(m => new VerificationMessage(MessageSeverity.Warning, m)).ToList());
        public static ModuleVerificationResult HasError(string message) => HasErrors(new List<string>() { message });
        public static ModuleVerificationResult HasErrors(List<string> messages) => FromMessages(messages.Select(m => new VerificationMessage(MessageSeverity.Error, m)).ToList());
        public static ModuleVerificationResult FromMessages(List<VerificationMessage> messages) => new ModuleVerificationResult(messages);
    }
